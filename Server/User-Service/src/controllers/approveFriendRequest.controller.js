...
const approveFriendRequest = async (req, res) => {
    try {
        ...
        // Add to friends list
        if (!user.friends.includes(requester.emailHash)) {
            user.friends.push(requester.emailHash);
        }
        if (!requester.friends.includes(user.emailHash)) {
            requester.friends.push(user.emailHash);
        }
